...
- runs the random effects, CIs and reports them
- add the fixed effects, CIs and reports them
- tests for influential studies
...
MetaAnalysis <- function(data) {
  ...
  res.Population <- rma(yi, vi, mods = cbind(factor(Population)), data = PitchMean, slab=Paper)
  res.Population
  confint(res.Population)
  forest(res.Population)
  ...
  # Clean results
  res_clean<- rma(yi, vi, data = PitchMean[c(1,3:8),], slab=Paper)
  res_clean
...
```{r}
### Calculate the effect sizes for the three relevant (>2 studies) variables:
